...
class VectorSearch {
    ...
    async recommendTools(query, mcpClient, modelName = null, options = {}) {
        try {
            // 使用默认模型名称
            const defaultModelName = modelName || process.env.DOUBAO_MODEL_NAME || 'doubao-embedding-text-240715';
    ...
    async searchSimilar(query, options = {}) {
        try {
            ...
            const {
                topK = 5,
                threshold = 0.1,
                modelName = process.env.DOUBAO_MODEL_NAME || 'doubao-embedding-text-240715'
            } = options;
